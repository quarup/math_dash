...
Player
  ...
  streakLevel           // answer streak 0..5 — +1 per correct, reset on wrong; scales coin pay (0.2 × level); persists across sessions
...
- **Single-currency model (revised 2026-09-08; supersedes the Phase-7 two-currency 🧱/🔬 design):** **coins** are spent on placements, land, and (later) map expansions / events. Coins come from every correct answer — `round(expectedSeconds × formatMult × 0.2 × streakLevel)` — plus a one-time band-crossing bonus (2× expectedSeconds) per concept per threshold. See *Domain Specs / Coin economy* and `lib/domain/economy/`.
...
- **`coin_economy.dart`** — the tunables and the rules:
  - correct answer pays `max(1, round(expectedSeconds × formatMult × kStreakStep × streakLevel))`, `formatMult` 1.0 MC / **1.5 keypad**; wrong pays 0;
  - `streakLevel` +1 per correct (cap **5** → full credit), reset to 0 on wrong; global per player, persisted (`Players.streakLevel`), computed at the *new* level so the first correct after a miss pays 20%;
